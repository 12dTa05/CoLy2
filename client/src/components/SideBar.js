...
const Sidebar = ({ isOpen, isCollapsed }) => {
  ...
  const mainMenuItems = [
    ...
    { path: '/subscriptions', icon: FaSubscript, label: 'Kênh đăng ký' }
  ];
  ...
  const renderMenuItem = (item, index) => (
    ...
    >
      <item.icon className={`sidebar-icon ${isCollapsed ? 'me-0' : 'me-3'}`} size={20} />
      ...
      <Nav className="flex-column">
        ...
        {currentUser && !isCollapsed && (
          <div className="sidebar-section">
            ...
            {/* Placeholder cho danh sách kênh đăng ký */}
